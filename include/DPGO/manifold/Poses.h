 ...
 * Internally store as r by (d+1)n matrix X = [X1, ... Xn], where each Xi = [Yi
 * pi] Each rotation Yi is a r-by-d matrix representing an element of the
 * Stiefel manifold Each translation pi is a r-dimensional vector
 */
...
public:
  ...
  /**
   * @brief Compute the average translation distance between two lifted pose
   * arrays Internally check that both arrays should have same dimension and
   * number of poses
   * @param poses1
   * @param poses2
   * @return
 ...
 * Internally store as d by (d+1)n matrix X = [X1, ... Xn], where each Xi = [Ri
 * ti] Each rotation Ri is a d-by-d matrix Each translation pi is a
...
public:
  ...
  // Covariance matrices associated with each pose
  Matrix Sigma_;
};
